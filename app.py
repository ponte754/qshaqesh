...
CATEGORIES = [
    ...
    'خدمات', 'حيوانات', 'ازياء', 'كتب', 'رياضة', 'هواتف', 'لابتوبات', 'اخرى' 
]
...
def home():
    ...
    try:
        all_ads = db.get_all_ads(current_username=None)
        for ad in all_ads:
            if ad.get('images'):
                try:
...
def public_view_ad(ad_id):
    try:
        ...
        print(f"Error in public_view_ad: {e}")
        return render_template('404.html'), 404
...
def index():
    ...
    for ad in all_ads:
        if ad.get('images'):
            ...
            ad['images_list'] = []
    return render_template('index.html', username=username, email=email, ads=all_ads, public_mode=False)
